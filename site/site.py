...
def find_results(query):
    ...
    for result in results:
        if '|' not in result.simple_phenotype:
            continue
        phenotypes = result.simple_phenotype.split('|')
        matching_phenotypes = [p for p in phenotypes if query.lower() in p.lower()]
        if len(matching_phenotypes) > 0:
            result.simple_phenotype = ", ".join(matching_phenotypes)
        else:
            result.simple_phenotype = ", ".join(phenotypes)
